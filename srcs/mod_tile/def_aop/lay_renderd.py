...
def main():
    ...
    for n in _data: 
        if n.find("_data") > 1:
            lyr_renderd.writelines(["\n", f"[{n[:-5].lower()}]\n",
                ...
                "TILESIZE=256\n",
                "MAXZOOM=20\n"])
            print_debug(f"[{n[:-5]}] Layer was added to renderd.conf !\n")
            lyr_renderd.writelines(["\n", f"[{n[:-5].lower()}_brd]\n",
                ...
                "TILESIZE=256\n",
                "MAXZOOM=20\n"])
            print_debug(f"[{n[:-5]}_brd] Layer was added to renderd.conf !\n")
            ...
            for line in fileinput.FileInput("/etc/apache2/sites-available/000-default.conf", inplace=True):
                ...
                print(line, end="")
            print_debug(f"Added the lines for both {n[:-5].lower()} and {n[:-5].lower()}_brd in the apache site config\n")
    ...
    for line in fileinput.FileInput("/var/www/html/index.html", inplace=True):
        ...
        print(line, end="")
    print_debug(f"Added the leaflist to index js\n")
